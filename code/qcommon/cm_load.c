...
static void CMod_LoadShaders( const lump_t *l ) {
	...
	cm.shaders = malloc( count * sizeof( *cm.shaders ), h_high );
	cm.numShaders = count;
...
static void CMod_LoadSubmodels( const lump_t *l ) {
	...
	cm.cmodels = malloc( count * sizeof( *cm.cmodels ), h_high );
	...
	for ( i=0 ; i<count ; i++, in++)
	{
		...
		indexes = malloc( out->leaf.numLeafBrushes * 4, h_high );
		...
		indexes = malloc( out->leaf.numLeafSurfaces * 4, h_high );
...
static void CMod_LoadNodes( const lump_t *l ) {
	...
	cm.nodes = malloc( count * sizeof( *cm.nodes ), h_high );
	cm.numNodes = count;
...
static void CMod_LoadBrushes( const lump_t *l ) {
	...
	cm.brushes = malloc( ( BOX_BRUSHES + count ) * sizeof( *cm.brushes ), h_high );
	cm.numBrushes = count;
...
static void CMod_LoadLeafs( const lump_t *l )
{
	...
	cm.leafs = malloc( ( BOX_LEAFS + count ) * sizeof( *cm.leafs ), h_high );
	cm.numLeafs = count;
	...
	cm.areas = malloc( cm.numAreas * sizeof( *cm.areas ), h_high );
	cm.areaPortals = malloc( cm.numAreas * cm.numAreas * sizeof( *cm.areaPortals ), h_high );
}
...
static void CMod_LoadPlanes( const lump_t *l )
{
	...
	cm.planes = malloc( ( BOX_PLANES + count ) * sizeof( *cm.planes ), h_high );
	cm.numPlanes = count;
...
static void CMod_LoadLeafBrushes( const lump_t *l )
{
	...
	cm.leafbrushes = malloc( (count + BOX_BRUSHES) * sizeof( *cm.leafbrushes ), h_high );
...
static void CMod_LoadLeafSurfaces( const lump_t *l )
{
	...
	cm.leafsurfaces = malloc( count * sizeof( *cm.leafsurfaces ), h_high );
...
static void CMod_LoadBrushSides( const lump_t *l )
{
	...
	cm.brushsides = malloc( ( BOX_SIDES + count ) * sizeof( *cm.brushsides ), h_high );
...
static void CMod_LoadEntityString( const lump_t *l ) {
	cm.entityString = malloc( l->filelen, h_high );
...
static void CMod_LoadVisibility( const lump_t *l ) {
	...
	if ( !len ) {
		...
		cm.visibility = malloc( cm.clusterBytes, h_high );
	...
	cm.vised = qtrue;
	cm.visibility = malloc( len, h_high );
...
static void CMod_LoadPatches( const lump_t *surfs, const lump_t *verts ) {
	...
	cm.surfaces = malloc( cm.numSurfaces * sizeof( cm.surfaces[0] ), h_high );
	...
	for ( i = 0 ; i < count ; i++, in++ ) {
		...
		cm.surfaces[ i ] = patch = malloc( sizeof( *patch ), h_high );
