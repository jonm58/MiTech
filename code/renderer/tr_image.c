...
static void Upload32( byte *data, int x, int y, int width, int height, image_t *image, qboolean subImage )
{
	...
	if ( scaled_width != width || scaled_height != height ) {
		if ( data ) {
			resampledBuffer = malloc( scaled_width * scaled_height * 4 );
...
image_t *R_CreateImage( const char *name, const char *name2, byte *pic, int width, int height, imgFlags_t flags ) {
	...
	image = malloc( sizeof( *image ) + namelen + namelen2, h_low );
...
qhandle_t RE_RegisterSkin( const char *name ) {
	...
	tr.numSkins++;
	skin = malloc( sizeof( skin_t ), h_low );
	tr.skins[hSkin] = skin;
	...
	if ( strcmp( name + strlen( name ) - 5, ".skin" ) ) {
		skin->numSurfaces = 1;
		skin->surfaces = malloc( sizeof( skinSurface_t ), h_low );
	...
	skin->surfaces = malloc( skin->numSurfaces * sizeof( skinSurface_t ), h_low );
...
void	R_InitSkins( void ) {
	...
	skin = tr.skins[0] = malloc( sizeof( skin_t ), h_low );
