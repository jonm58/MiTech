...
static shader_t *GeneratePermanentShader( void ) {
	...
	newShader = malloc( sizeof( shader_t ), h_low );
	...
	*newShader = shader;
	...
	for ( i = 0 ; i < newShader->numUnfoggedPasses ; i++ ) {
		...
		newShader->stages[i] = malloc( sizeof( stages[i] ), h_low );
		*newShader->stages[i] = stages[i];

		for ( b = 0 ; b < NUM_TEXTURE_BUNDLES ; b++ ) {
			size = newShader->stages[i]->bundle[b].numTexMods * sizeof( texModInfo_t );
			if ( size ) {
				newShader->stages[i]->bundle[b].texMods = malloc( size, h_low );
...
static void ScanAndLoadShaderFiles( void )
{
	...
	s_shaderText = malloc( sum + numShaderFiles*2 + 1, h_low );
	...
	hashMem = malloc( size * sizeof(char *), h_low );
